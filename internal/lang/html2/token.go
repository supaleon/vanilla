...
const (
	...
	LBRACE   // {
	RBRACE   // }
	Colon    // : -> {unaryBooleanExpression : $defaultText} -> {isLoggedIn : Welcome back!}
	Mod      // % -> {unaryBooleanExpression % $formatTemplate} -> {currentTime % YY-MM-DD H:M:S}
)
...
type Token struct {
	Type     TokenType
	DataAtom atom.Atom
	Data     string // -> textToken, commentToken
	Attr     []Attribute
}
...
func (z *Tokenizer) readStartTag() TokenType {
	...
	if raw {
		z.rawTag = toLower(string(z.buf[z.data.start:z.data.end]))
	}
