...
// Offset is a compact encoding of a source position within a file set.
// It can be converted into a [Position] for a more convenient, but much
// larger, representation.
//
// The Offset value for a given file is a number in the range [base, base+size],
// where base and size are specified when a file is added to the file set.
// The difference between a Offset value and the corresponding file base
// corresponds to the byte offset of that position (represented by the Offset value)
// from the beginning of the file. Thus, the file base offset is the Offset value
// representing the first byte in the file.
//
// To create the Offset value for a specific source offset (measured in bytes),
// first add the respective file to the current file set using [FileSet.AddFile]
// and then call [File.Pos](offset) for that file. Given a Offset value p
// for a specific file set fset, the corresponding [Position] value is
// obtained by calling fset.Position(p).
//
// Offset values can be compared directly with the usual comparison operators:
// If two Offset values p and q are in the same file, comparing p and q is
// equivalent to comparing the respective source file offsets. If p and q
// are in different files, p < q is true if the file implied by p was added
// to the respective file set before the file implied by q.
type Offset int

// The zero value for [Offset] is NoOffset; there is no file and line information
// associated with it, and NoPos.IsValid() is false. NoOffset is always
// smaller than any other [Offset] value. The corresponding [Position] value
// for NoOffset is the zero value for [Position].
const NoOffset Offset = 0

// IsValid reports whether the position is valid.
func (p Offset) IsValid() bool {
	return p != NoOffset
}
...
// LineStart returns the [Offset] value of the start of the specified line.
// It ignores any alternative positions set using [File.AddLineColumnInfo].
// LineStart panics if the 1-based line number is invalid.
func (f *File) LineStart(line int) Offset {
	if line < 1 {
		panic(fmt.Sprintf("invalid line number %d (should be >= 1)", line))
	}
	f.mutex.Lock()
	...
	return Offset(f.base + f.lines[line-1])
}
...
// Offset returns the Offset value for the given file offset.
//
...
// f.Pos(f.Offset(p)) == p.
func (f *File) Pos(offset int) Offset {
	return Offset(f.base + f.fixOffset(offset))
}

// Offset returns the offset for the given file position p.
//
// If p is before the file's start position (or if p is NoOffset),
...
func (f *File) Offset(p Offset) int {
	return f.fixOffset(int(p) - f.base)
}

// Line returns the line number for the given file position p;
// p must be a [Offset] value in that file or [NoOffset].
func (f *File) Line(p Offset) int {
...
func (f *File) position(p Offset, adjusted bool) (pos Position) {
...
// p must be a Offset value in f or NoOffset.
func (f *File) PositionFor(p Offset, adjusted bool) (pos Position) {
	if p != NoOffset {
...
func (f *File) Position(p Offset) (pos Position) {
...
// is the corresponding file size. A [Offset] value is a value in such
// an interval. By determining the interval a [Offset] value belongs
// to, the file, its file base, and thus the byte offset (position)
// the [Offset] value is representing can be computed.
//
// When adding a new file, a file base must be provided. That can
// be any integer value that is past the end of any interval of any
// file already in the file set. For convenience, [FileSet.Base] provides
// such a value, which is simply the end of the Offset interval of the most
...
// exists between a [Offset] value p for a given file offset offs:
//
//	int(p) = base + offs
//
...
// queries for its [Offset] interval yield a negative result.
...
func (s *FileSet) file(p Offset) *File {
...
// If no such file is found (for instance for p == [NoOffset]),
// the result is nil.
func (s *FileSet) File(p Offset) (f *File) {
	if p != NoOffset {
		f = s.file(p)
	}
	return
...
// PositionFor converts a [Offset] p in the fileset into a [Position] value.
// If adjusted is set, the position may be adjusted by position-altering
// //line comments; otherwise those comments are ignored.
// p must be a [Offset] value in s or [NoOffset].
func (s *FileSet) PositionFor(p Offset, adjusted bool) (pos Position) {
	if p != NoOffset {
		if f := s.file(p); f != nil {
...
// Position converts a [Offset] p in the fileset into a Position value.
// Calling s.Position(p) is equivalent to calling s.PositionFor(p, true).
func (s *FileSet) Position(p Offset) (pos Position) {
