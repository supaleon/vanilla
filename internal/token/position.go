...
// NB: The code is copied from the `go/token/token.go`.
...
// Loc is a compact encoding of a source position within a file set.
// It can be converted into a [Position] for a more convenient, but much
// larger, representation.
//
// The Loc value for a given file is a number in the range [base, base+size],
// where base and size are specified when a file is added to the file set.
// The difference between a Loc value and the corresponding file base
// corresponds to the byte offset of that position (represented by the Loc value)
// from the beginning of the file. Thus, the file base offset is the Loc value
// representing the first byte in the file.
//
// To create the Loc value for a specific source offset (measured in bytes),
// first add the respective file to the current file set using [FileSet.AddFile]
// and then call [File.ToLocation](offset) for that file. Given a Loc value p
// for a specific file set fset, the corresponding [Position] value is
// obtained by calling fset.Position(p).
//
// Loc values can be compared directly with the usual comparison operators:
// If two Loc values p and q are in the same file, comparing p and q is
// equivalent to comparing the respective source file offsets. If p and q
// are in different files, p < q is true if the file implied by p was added
// to the respective file set before the file implied by q.
type Loc int

// NoLoc is zero value for [Loc]; there is no file and line information
// associated with it, and NoPos.IsValid() is false. NoLoc is always
// smaller than any other [Loc] value. The corresponding [Position] value
// for NoLoc is the zero value for [Position].
const NoLoc Loc = 0

// IsValid reports whether the position is valid.
func (p Loc) IsValid() bool {
	return p != NoLoc
}
...
type File struct {
	...
	base int    // Location value range for this file is [base...base+size]
...
// LineStart returns the [Loc] value of the start of the specified line.
// It ignores any alternative positions set using [File.AddLineColumnInfo].
// LineStart panics if the 1-based line number is invalid.
func (f *File) LineStart(line int) Loc {
	if line < 1 {
		panic(fmt.Sprintf("invalid line number %d (should be >= 1)", line))
	}
	f.mutex.Lock()
	...
	return Loc(f.base + f.lines[line-1])
}
...
// Location returns the Loc value for the given file offset.
//
// If the offset is negative, the result is the file's start
// position; if the offset is too large, the result is
// the file's end position (see also go.dev/issue/57490).
//
// The following invariant, though not true for Loc values
// in general, holds for the result p:
// f.Pos(f.Offset(p)) == p.
func (f *File) Location(offset int) Loc {
	return Loc(f.base + f.fixOffset(offset))
}

// Offset returns the offset for the given file position p.
//
// If p is before the file's start position (or if p is NoLoc),
...
func (f *File) Offset(p Loc) int {
	return f.fixOffset(int(p) - f.base)
}

// Line returns the line number for the given file position p;
// p must be a [Loc] value in that file or [NoLoc].
func (f *File) Line(p Loc) int {
...
func (f *File) position(p Loc, adjusted bool) (pos Position) {
...
// p must be a Loc value in f or NoLoc.
func (f *File) PositionFor(p Loc, adjusted bool) (pos Position) {
	if p != NoLoc {
...
func (f *File) Position(p Loc) (pos Position) {
...
// is the corresponding file size. A [Loc] value is a value in such
// an interval. By determining the interval a [Loc] value belongs
// to, the file, its file base, and thus the byte offset (position)
// the [Loc] value is representing can be computed.
//
// When adding a new file, a file base must be provided. That can
// be any integer value that is past the end of any interval of any
// file already in the file set. For convenience, [FileSet.Base] provides
// such a value, which is simply the end of the Loc interval of the most
...
// exists between a [Loc] value p for a given file offset offs:
//
//	int(p) = base + offs
//
// with offs in the range [0, size] and thus p in the range [base, base+size].
// For convenience, [File.ToLocation] may be used to create file-specific position
...
// queries for its [Loc] interval yield a negative result.
...
func (s *FileSet) file(p Loc) *File {
...
// If no such file is found (for instance for p == [NoLoc]),
// the result is nil.
func (s *FileSet) File(p Loc) (f *File) {
	if p != NoLoc {
		f = s.file(p)
	}
	return
...
// PositionFor converts a [Loc] p in the fileset into a [Position] value.
// If adjusted is set, the position may be adjusted by position-altering
// //line comments; otherwise those comments are ignored.
// p must be a [Loc] value in s or [NoLoc].
func (s *FileSet) PositionFor(p Loc, adjusted bool) (pos Position) {
	if p != NoLoc {
		if f := s.file(p); f != nil {
...
// Position converts a [Loc] p in the fileset into a Position value.
// Calling s.Position(p) is equivalent to calling s.PositionFor(p, true).
func (s *FileSet) Position(loc Loc) (pos Position) {
	return s.PositionFor(loc, true)
}
